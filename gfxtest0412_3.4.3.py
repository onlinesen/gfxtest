...
class MyLogger(type(sys)):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.output = str(args[0]) + "_" + time.strftime("%m%d%H%M%S",
                                                         time.localtime()) + ".txt"
...
class GFXTest():
    ...
    def gettk(self):
        ...
        settingslable = Label(self.root, bg="MediumAquamarine", text='请手动打开设置中的GPU Rendering!', font=("Century"),
                              fg="Crimson")
        settingslable.config(image=photoimgbottom)
        ...
        crop.config(image=photocropimg)
        ...
        savebutton.config(image=saveimgphoto)
        ...
        openbutton.config(image=openimgphoto)
        ...
        blanko.config(image=photo51)
        blanko.place(x=660, y=40, width=80, height=30)
        ...
        edit_menu.add_command(label="启用脚本编辑模式", command=self.control_openfile)
        ...
        # self.mytouch = MyTouch.MinitouchStreamMixin()
    ...
    def pressure_test(self):
        try:
            self.logger = MyLogger("Press_Log")
            time.sleep(0.1)
            self.screensave = int(self.numberChosen.get())
            ...
            if not os.path.isfile(getfile):
                ...
                self.textout.update()
                return 0
            # self.raw_cmd('shell','monkey', '-p', 'com.github.uiautomator', '-c', 'android.intent.category.LAUNCHER', '1')
            # self.d.press.back()
            try:
            ...
            for i in xrange(1, int(self.screensave) + 1):
                self.logger.debug("-- " + str(i) + " -- " + str(shotname) + " start")
                if self.stop_flag:
                    return 0
                else:
                    self.textout.insert(1.0, str(i) + "> start... \n")
                    self.textout.update()
                    self.playatxthread(getfile)
                    self.logger.debug("-- " + str(i) + " -- end")
                    # self.d.freeze_rotation(0)
    ...
    def getLog(self):
        try:
            ...
            self.textout.insert(END, "getLog 出错了\n")
            self.textout.update()
        finally:
            self.textout.insert(END, "getLog 结束.\n")
            self.textout.update()
    ...
    def installbundle(self):
        try:
            ...
            out1 = subprocess.check_output(
                "adb -s " + self.serial + " shell ls /data/local/tmp/bundle.jar; exit 0",
            ...
            out = subprocess.check_output(
                "adb -s " + self.serial + " shell ls /data/local/tmp/uiautomator-stub.jar; exit 0",
                stderr=subprocess.STDOUT, shell=True)
            if "No such" in out:
                self.raw_cmd('push', os.getcwd() + '/lib/bundle/uiautomator-stub.jar',
                             '/data/local/tmp/')
            out = subprocess.check_output(
                "adb -s " + self.serial + " shell ls /data/local/tmp/busybox; exit 0",
                stderr=subprocess.STDOUT, shell=True)
            if "No such" in out:
            ...
            self.raw_cmd('push', os.getcwd() + '/lib/' + cpu + '/minitouch',
                         '/data/local/tmp/')
            self.raw_cmd('shell', 'chmod', '777', '/data/local/tmp/minitouch')
            return True
        except Exception, e:
            return False
    ...
    def permissionClick(self, xy, serial):
        os.system('adb -s ' + serial + " wait-for-device shell input tap " + str(xy[0]) + " " + str(xy[1]))
        time.sleep(0.2)
    ...
    def playatxthread(self, playfile):
        if ".txt" in playfile:
            t = threading.Thread(target=self.playatxfile, args=(playfile,))
            t.setDaemon(True)
            t.start()
            t.join()
        elif ".json" in playfile:
            tcs = self.parser_test_case(playfile)
            t = threading.Thread(target=self.playatxfile, args=(tcs,))
            t.setDaemon(True)
            t.start()
            t.join()

    def playatxfile(self, playfile):
        try:
            rt = ""
            if type(playfile) == dict:
                for i in playfile["Testcases"]:
                    self.logger.debug("TestCaseID:" + str(i["ID"]))
                    # self.logger.debug("TestDevice:" + str(i["Device"]))
                    with open(i["URL"], 'a+') as f:
                        lines = f.read()
                        readline = lines.split("\n")
                        rt = self.playatxcontent(readline)
                        self.logger.debug("Result: " + str(rt))
                self.textout.insert(1.0, "Result: " + str(self.result) + "\n")
                self.textout.update()
                ...
                with open(playfile, 'a+') as f:
                    ...
                    rt = self.playatxcontent(readline)
                    self.logger.debug("Result: " + str(rt))
                    self.textout.insert(1.0, "Result: " + str(rt) + "\n")
                    self.textout.update()
        except Exception, e:
            import traceback
            traceback.print_exc()
    ...
    def playatxcontent(self, playcontent):
                ...
                if line != "":
                    if loopflag and "end" not in line:
                        ...
                        for i in xrange(0, sub_content_times):
                            self.playatxcontent(sub_content)
                        ...
                        if playscreenx > self.WIDTH:
                            x = float(x) * playscreenx / self.WIDTH
                            x1 = float(x1) * playscreenx / self.WIDTH
                        else:
                            x = float(x) * self.WIDTH / playscreenx
                            x1 = float(x1) * self.WIDTH / playscreenx
                        if playscreeny > self.HEIGHT:
                            y = float(y) * playscreeny / self.HEIGHT
                            y1 = float(y1) * playscreeny / self.HEIGHT
                        else:
                            y = float(y) * self.HEIGHT / playscreeny
                            y1 = float(y1) * playscreeny / self.HEIGHT
                        if "drag" in line:
                        ...
                        self.d.screenshot(
                            "pic/takeshot_" + time.strftime("%m%d%H%M%S", time.localtime()) + ".png")

                    elif "checktext" in line:
                        op = "checktext"
                        x = line.split(":")[1]
                        p = self.d(text=x).wait.exists(timeout=5000)
                        print "checktext:", x
                        if p:
                            self.textout.insert(1.0, "Text <" + x + ">" + " < Found!> \n")
                            self.textout.update()
                            self.logger.info("checktext:" + x + "  Found!>")
                            self.d.screenshot(
                                "pic/checktext_Found_" + time.strftime("%m%d%H%M%S", time.localtime()) + ".png")

                        else:
                            self.result = False
                            self.logger.error("checktext:" + x + ",  Not Found!>")
                            self.textout.insert(1.0, "Text <" + x + ">" + " < Not Found!>\n")
                            self.textout.update()
                            self.d.screenshot(
                        ...
                        time.sleep(1)
                        x = line[line.find(":") + 1:]
                        getORno = self.dm.wait(x, timeout=10)
                        print "checkimage:" + str(x) + "-->" + str(getORno)
                        if getORno is None:
                            self.result = False
                            self.textout.insert(1.0, "Image <" + x + ">" + " < Not Found! " + " >\n")
                            self.textout.update()
                            self.logger.error("checkimage:" + x + ", Not Found! ")
                            self.d.screenshot(
                            ...
                            if type(conf) == tuple:
                                self.logger.info(
                                    "checkimage:" + x + ", Found=[" + str() + "],Confidence=" + str(getORno[1]))
                            else:
                                self.logger.info("checkimage:" + x + ", Found=[" + str() + "],Confidence=" + str(
                                    '%.3f' % getORno[1]))
                            # self.result = "Found=[" + str(getORno[0]) + "],Confidence=" + str('%.3f' % getORno[1])
                            # self.textout.insert(END,
                            #                     "Image <" + x + ">" + " < Found at: " + str(getORno[0]) +  ",confidence=" + str(getORno[1])+" >\n")
                            self.d.screenshot(
                        ...
                        try:
                            ...
                            time.sleep(1)
                            self.d.wait.idle()
                            self.logger.info("clicktext:" + x)
                        except Exception, jc:
                            self.result = False
                            self.textout.insert(1.0, "点击文字出错：" + x + "\n")
                            self.textout.update()
                            self.d.screenshot(
                        ...
                        x = line[line.find(":") + 1:]
                        getORno = self.dm.wait(x, timeout=10)
                        print "getorno=", getORno
                        if getORno is None:
                            self.result = False
                            self.textout.insert(1.0, "Click image: " + x + " Fail! " + " >\n")
                            self.textout.update()
                            self.logger.error("image:" + x + ", Not Found! ")
                            self.d.screenshot(
                                "pic/imagenotfound_" + time.strftime("%m%d%H%M%S", time.localtime()) + ".png")
                        elif getORno.matched:
                            self.textout.insert(1.0, "Click image: " + x + " OK!" + " >\n")
                            self.textout.update()
                            subprocess.Popen(
                                'adb -s ' + self.serial + " shell input tap " + str(getORno[0][0]) + " " + str(
                                    getORno[0][1]), stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
                            self.logger.debug("clickimage:" + x + ":" + str(getORno[0][0]) + "_" + str(getORno[0][1]))
    ...
    def _stroke_done(self, event):
        try:
            ...
            # print self._mouse_motion,stend ,self.cavas_x_y.keys()
            ...
            if self._mouse_motion == "click":
                ...
                if ((stend - ststart[0]).total_seconds() > 0.35) and (len(ststart) > 0):
                ...
                if len(self._mouse_motion_xy) >= 2:
                    ...
                    if self._mouse_motion_crop == "crop":
                        ...
                        try:
                            ...
                            # traceback.print_exc()
                            pass
                    elif len(self._mouse_motion_xy) < 8:
                        # if abs(y_end - y_start) > 50 or abx(x_end - x_start) >50:
                        if self.minitch_flag:
                            self.minitouch.swipe((x_start, y_start), (x_end, y_end))
                        else:
                            self.raw_cmd_nowait('shell', ' input', 'swipe', str(x_start), str(y_start), str(x_end),
                                                str(y_end))
    ...
    def control_edit(self, mode="clicktext"):
        ...
        if mode == "clicktext":
            ...
            self.textout.insert(END, "longclickscreen:200x300\n")
        elif mode == "screenon":
            self.textout.insert(END, "screenon:on/off \n")
        elif mode == "takeshot":
            self.textout.insert(END, "takeshot:1 \n")
            ...
            self.textout.insert(END, "install:app.apk\n")
        elif mode == "uninstall":
            self.textout.insert(END, "uninstall:com.android.app\n")
        elif mode == "loop":
            self.textout.insert(INSERT, "loop:3 \n")
            self.textout.insert(INSERT, "<loopcode> \n")
            self.textout.insert(INSERT, "endloop\n")
        elif mode == "launch":
            self.textout.insert(END, "launch:com.android.app\n")
        self.textout.update()
    ...
    def control_openfile(self):
        ...
        if ref == "":
            ...
            ref = tkinter.filedialog.askopenfilenames(
    ...
    def control_save(self, event):
        try:
            ...
            print "save as:", ref
            if ref == "":
                ref = tkinter.filedialog.asksaveasfilename(filetypes=[("text file", "*.txt"), ("all", "*.*")])
                if len(ref) < 1:
                    return
                if ".txt" not in ref:
                    ref = ref + ".txt"
            ...
            if tkinter.messagebox.askokcancel('提示', '是否保存到Json 用例文件？'):
                fn = tkinter.filedialog.askopenfilenames(filetypes=[("json file", "*.json"), ("all", "*.*")])
                if len(fn) == 0:
                    return
                json_dict = self.parser_test_case(fn[0])
                tc = json_dict["Testcases"]
                (path, shotname) = os.path.split(ref)
                if tc != None:
                    json_dict["Testcases"].append({"ID": shotname, "URL": glob.glob(ref)[0]})
                # jsfile = io.open(fn[0], 'w', encoding='utf8')
                with open(fn[0], 'w') as jsfile:
                    json.dump(json_dict, jsfile)
                    self.textout.insert(END, "Json 用例保存完成")
                    self.textout.update()
    ...
    def install_app(self):
        ...
        if len(serial) > 1:
            if tkinter.messagebox.askokcancel('提示', '所有设备安装点<是>,<否>仅安装到当前设备:' + self.serial):
                pass
            else:
                serial = [self.serial]
        for fl in (fn):
            for i in xrange(0, len(serial)):
                self.serial = serial[i]
                self.textout.insert(END, '安装: ' + str(fl) + '\n')
                self.textout.update()
