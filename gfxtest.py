...
wm dismiss-keyguard 解锁
svc power stayon true 亮屏
  所以FPS的算法可以变为：
  m / （m + 额外的垂直同步脉冲） * 60
  '''
...
class GFXTest(object):
    ...
    def gettk(self):
        ...
        for i in psutil.pids():
            try:
                ...
                continue
        self.shouwport = "131" + str(minicapport / 2)
        self.touchport = int('111' + str(int(minicapport / 2)))
        self.radionButton_value = StringVar()
        for i in xrange(len(serial)):
            self.radionButton_value.set(serial[int(minicapport / 2) - 1])
        ...
        if len(serial) == 1:
            ...
            size = self.screenSize(self.serial)
            if not size:
                tkMessageBox.askokcancel('提示', '请重连设备！')
            self.WIDTH = int(size[0])
            self.HEIGHT = int(size[1])
            self.apkversion, self.buildversion, self.targetSDK = self.getAPKVersion(self.package)
            self.textout.insert(END, "Platform:" + self.buildversion + "\n")
            ...
            tkMessageBox.askokcancel('提示', '没有连接设备，请连接设备后重启！')
        ...
        onekey_menu.add_command(label="界面元素窥视模式", command=self.study_mode_threading)
        onekey_menu.add_command(label="注册监视器", command=self.watch_define_threading)
        ...
        edit_menu.add_command(label="输入文字", command=lambda: self.control_edit("settext"))
        ...
        self.minicap_ins = MyMini.MyMini(serial=self.serial, tk=self.root, cav=self.canvas, port=self.shouwport)
        self.minicap_ins.screen_simple()
        ...
        self.minitouch = MiniTouch.Minitouch(self.touchport)
        self.minitouch.teardown()
        self.minitouch_thread(self.touchport)
    ...
    def minitouch_thread(self, port):
        try:
            cmd = ('adb -s ' + self.serial + ' forward tcp:' + str(port) + ' localabstract:minitouch')
    ...
    def execute_stop(self):
        if tkMessageBox.askokcancel('提示', '是否终止执行？'):
            self.stop_flag = True
        else:
            self.stop_flag = False
    ...
    def on_minicap_killed(self):
        if tkMessageBox.askokcancel('提示', '是否断开当前设备的屏幕显示？' + self.serial):
            self.minicap_ins.killMinicap()
            self.raw_cmd('shell', 'input', 'keyevent 4')
            self.raw_cmd('forward', '--remove', 'tcp:' + str(self.touchport))
            self.killMinitouch()
            self.textout.delete(1.0, END)
    ...
    def on_minicap_reconnect(self):
        ...
        self.textout.insert(1.0, "Reconnect device:" + self.radionButton_value.get() + "\n")
        self.textout.update()
        self.minicap_ins.open_minicap_stream(serial=self.serial)
        self.minicap_ins.flag = True
        self.minicap_ins.screen_simple()

        #
        # del self.minitouch
        # self.minitouch = MiniTouch.Minitouch(touchport)
        self.minitouch_thread(self.touchport)
        self.minitouch.install_and_setup(self.serial, self.WIDTH, self.HEIGHT)

    def on_super_replay(self):
        tkMessageBox.showinfo(title="提示框",
                                    message="输入测试脚本文件:\n" + "[功能列表：]\n"
                                            + "sleep\n"
                                            + "presshome\n"
                                            + "pressback\n"
    ...
    def on_recordreplay_record(self):
        ...
        if self.fileEdit.get() == "":
            tkMessageBox.showinfo(title="提示框", message="录制回放可以输入文件，默认temp.txt \n 请点击[START]开始！")
    ...
    def installbundle(self, serial):
        try:
            ...
            outinstall = subprocess.Popen(
                ('adb', '-s', serial, 'shell','pm', 'list', 'package', 'com.github.uiautomator'),
                stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
            if "com.github.uiautomator" not in outinstall:
                cmd =['adb', '-s', serial, 'install', '-r', os.getcwd() + '/lib/bundle/app.apk']
                ...
                subprocess.Popen(
                    ...
                    stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
            # self.raw_cmd('am', 'start','-W','com.tinno.uiautomator/com.tinno.uiautomator.MainActivity')
            ...
            if "No such" in out:
                ...
                cpu = subprocess.Popen(cmd,shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
                cpu = cpu.strip()
                cmd = 'adb -s ' + serial + ' shell getprop ro.build.version.sdk'
                sdk = subprocess.Popen(cmd,shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
                sdk = sdk.strip()
                os.system('adb '+ ' -s '+ serial+ ' push '+ os.getcwd() + '/lib/' + sdk + '/' + cpu + '/minicap.so'+ ' /data/local/tmp/')
                os.system('adb '+ ' -s ' + serial + ' push ' + os.getcwd() + '/lib/' + sdk + '/' + cpu + '/minicap' + ' /data/local/tmp/')
                cmd = 'adb -s ' + serial + ' shell chmod 777 /data/local/tmp/minicap'
                subprocess.Popen(cmd,shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                os.system('adb ' + ' -s ' +  serial + ' push ' + os.getcwd() + '/lib/' + cpu + '/minitouch'+' /data/local/tmp/')
                cmd = 'adb -s ' + serial + ' shell chmod 777 /data/local/tmp/minitouch'
                subprocess.Popen(cmd,shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
            cmd = ('adb -s ' + self.serial + ' forward tcp:9008 tcp:9008')
            subprocess.Popen(cmd, shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            self.uiautomator_process = subprocess.Popen(['adb', '-s', serial, 'shell', "am", "instrument", "-w",
                              "com.github.uiautomator.test/android.support.test.runner.AndroidJUnitRunner"],
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            # cmds = 'adb -s ' + serial + ' shell am start -W com.github.uiautomator/.MainActivity'
    ...
    def screenSize(self, serial):
        try:
            ...
            print "No any device!"
            tkMessageBox.askokcancel('提示', '请adb devices查看连接的设备，设备异常！')
            # self.textout.insert("1.0", "No any device found!\n")
            sys.exit(1)
    ...
    def run_monkey(self):
        serial = [self.serial]
        if tkMessageBox.askokcancel('提示', '运行所有设备点<是>,<否>仅仅执行选择的设备'):
            serial = self.getAdb2()
        else:
            pass
    ...
    def chang_language(self):
        if tkMessageBox.askokcancel('提示', '目录中lib/bundle/language_config.ini为语言配置文件！'):
            self.killlc()
        ...
        try:
            ...
            if package_Edit == "":
                self.raw_cmd('shell', 'monkey',
                             ...
                             '--ignore-crashes', '--ignore-timeouts', '--ignore-native-crashes', '-v', '2000000000',
    ...
    def killmonkey(self):
        try:
            serial = [self.serial]
            if tkMessageBox.askokcancel('提示', '停止所有设备点<是>,<否>仅仅停止选择的设备'):
                serial = self.getAdb2()
            else:
                pass
            for i in xrange(0, len(serial)):
                ...
                for j in out.split("\n"):
                    if " " in j:
                        ps = Popen("adb  -s " + self.serial + " shell kill " + j.split()[0], shell=True, stdout=PIPE,
                                   stderr=PIPE)
                        ps.communicate()
                ...
                for k in out.split("\n"):
                    if " " in k:
                        ps = Popen("adb  -s " + self.serial + " shell kill " + k.split()[0], shell=True, stdout=PIPE,
                                   stderr=PIPE)
                        ps.communicate()
    ...
    def platformRun2(self):
        if tkMessageBox.askokcancel('提示', '要执行GO整机测试吗？'):
            import ConfigParser
            import glob
            import csv
            self.inidevice()
    ...
    def playatxfile(self, playfile, rtime, count):
        try:
            serial = self.getAdb2()
            for i in xrange(0, len(serial)):
                cmd = ('adb -s ' + serial[i] + ' forward tcp:9008 tcp:9008')
                subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                subprocess.Popen(['adb', '-s', serial[i], 'shell', "am", "instrument", "-w",
                                  "com.github.uiautomator.test/android.support.test.runner.AndroidJUnitRunner"],
                                 stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                # cmds = 'adb -s ' + serial[i] + ' shell am start -W com.github.uiautomator/.MainActivity'
                # subprocess.Popen(cmds, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                # time.sleep(0.5)
                # subprocess.Popen(['adb', '-s', serial[i], 'shell', 'input', 'keyevent', '4'],
                #                  stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                # time.sleep(0.5)
            try:
                self.d = Device(self.serial)
                self.watch_threading()
            except Exception, e:
                print self.d.info
            reporttime = rtime
            serial_b = None
            if type(playfile) == dict:
                for i in playfile["Testcases"]:
                    ...
                    self.playatxcontent(readline, serial_b, reporttime)
                    self.logger.debug(
                ...
                if line != "":
                    ...
                    if loopflag and "end" not in line:
                        ...
                        picnameb = os.getcwd() + "/pic/" + reporttime + "/" + time.strftime("%m%d%H%M%S",
                            ...
                            index) + "_BeforClick_" + str(x) + "x" + str(y) + ".png"

                        imgsrc = Image.open(self.d.screenshot(str(self.d.server.adb.device_serial()) + "_tmp.png"))  #
                        ...
                        if "long" in line:
                            self.minitouch.touch((x, y), duration=2)
                            self.minitouch.touch_end()
                        else:
                            self.minitouch.touch((x, y))
                            self.minitouch.touch_end()
                        time.sleep(1)
                        picname = os.getcwd() + "/pic/" + reporttime + "/" + time.strftime("%m%d%H%M%S",
                                                                                           time.localtime()) + "_" + str(
                            index) + "_AfterClick_" + str(x) + "x" + str(y) + ".png"
                        self.textout.insert(1.0,
                                            " ." + str(index) + ". " + serial + " Click Screen:" + str(x) + ',' + str(
                                                y) + " Pass\n")
                        self.logger.info(str(index) + " " + serial + " " + str(line) + " Pass")
                        self.stepresult[-1][1].append(
                            {"step": index, "tcs": "Click screen:" + str(x) + "," + str(y), "result": "Pass",
                        ...
                        self.d.screenshot(
                            ...
                            quality=50)
                    elif "settext" in line:
                        x = line.split(":")[1]
                        cmds = 'adb -s ' + self.serial + ' shell input text ' + x
                        subprocess.Popen(cmds, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        time.sleep(1)
                    elif "getcolor" in line:
                        x = line.split(":")[1]
                        b = self.d(resourceId=x).bounds
                        ...
                        img = img.crop(box)
                        color = GetMainColor.get_dominant_color(img)
                        get = GetMainColor.min_color_diff(color)
                        if len(get) == 2:
                            result &= True if get[1] == "GREEN" else False
                            print get[1]
                        ...
                        if "." in fn and "x" in fn:
                            ...
                            if irx != int(device_s[serial][0]) or iry != int(device_s[serial][1]):
                                ...
                                getORno = self.find_template(d=self.d, img=x1, wt=wtimeout, threshold=th)
                            else:
                                getORno = self.find_template(d=self.d, img=x, wt=wtimeout, threshold=th)
                        ...
                        draw = ImageDraw.Draw(imgsrc)
                        if getORno == None:
                            self.logger.info(str(index) + " " + serial + " Check Image:" + fn.split(".")[0] + "Fail")
                            self.textout.insert(1.0,
                            ...
                            # time.sleep(0.2)
                            self.logger.info(
                                str(index) + " " + serial + " Image Found:" + fn.split(".")[0] + " Pass at: [" + str(
                                    xy_x) + "," + str(xy_y) + "," + str(xy_x1) + "," + str(xy_y1) + "]")
                            self.textout.insert(1.0,
                                                " ." + str(index) + ". " + serial + " Check Image:" + fn.split(".")[
                                                    0] + " Pass\n")
                        ...
                        op = "scrolltotext"
                        x = line[line.find(":") + 1:]
                        print "scroll to text: " + x

                        result = self.d(scrollable=True).scroll.to(text=x)
                        self.textout.insert(1.0, " ." + str(index) + ". " + serial + " scroll to text:" + x)
                        # self.d(text=x).click()
                        time.sleep(1)
                        ...
                        if "." in fn and "x" in fn:
                            ...
                            if irx != int(device_s[serial][0]):
                                ...
                                if not os.path.exists(x1):
                                    ...
                                    img = img.resize((int(w), int(h)), Image.ANTIALIAS)
                                    img.save(x1.strip())
                                getORno = self.find_template(d=self.d, img=x1, wt=wtimeout)
                            else:
                                getORno = self.find_template(d=self.d, img=x, wt=wtimeout)
                        imgsrc = Image.open(self.d.screenshot(str(self.d.server.adb.device_serial()) + "_tmp.png"))
                        # imgsrc = self.minicap_ins.crop_image()
                        if imgsrc == None:
                            continue
                        ...
                        if ":id" in x:
                            ...
                            i1 = pkg + ":id/" + im1[0]
                            i2 = pkg + ":id/" + im1[1]
                            ssim = self.compareImage(i1, i2, li=False, bc=back)
                        else:  # 第一个参数为本地图片，第2个参数为手机资源id
                            ssim = self.compareImage(im1[0], im1[1], li=True, bc=back)
                        print ssim
                        if ssim > 0.7:
                            result = True
                        else:
    ...
    def find_template(self, d, img, threshold=0.5, rgb=False, wt=10):
        """函数功能：找到最优结果."""
        ...
        for i in xrange(0, wt):
            try:
                # 第一步：校验图像输入
                im_source = ac.imread(
                    dv.screenshot(os.getcwd() + "/" + str(dv.server.adb.device_serial()) + "_tmp.png"))
                ...
                if best_match['result'][0] >= 1 and best_match['result'][1] >= 1 and confidence >= threshold:
                    ...
                    self.logger.info("图片检查精度：" + str("%0.3f" % (best_match['confidence'])) + " 位置:" + str(
                        best_match['result']))
                    self.textout.insert(1.0,
    ...
    def travelApp(self, pkg):
        try:
            ...
            time.sleep(1)
            nomd, xy, an = self.dumnode(self.serial)
            if pkg not in self.getPackage():
                for pi in xy.keys():
                    perkey.append(xy.get(pi))
                for i in xrange(0, 6):
                    if pkg not in self.getPackage():
                        self.permissionClick(max(perkey), self.serial)
            time.sleep(2)
            nomd, xy, an = self.dumnode(self.serial)
            base = xy
            nomdo = nomd
            ...
            while ct <= int(590) and (len(xy) > 0) and runtflat:
                ...
                time.sleep(0.2)
                nomdn, xy, an = self.dumnode(self.serial)
    ...
    def _mouse_click(self, event):
        ...
        if self._mouse_motion_crop == "crop":
            ...
            # print "Touch mode"
            try:
                self.minitouch.touch((click_x, click_y), 0.1)
            except Exception,e:
                os.system("adb -s " + self.serial + " shell input tap " + str(click_x) + " " + str(click_y))
    ...
    def _stroke_done(self, event):
        try:
            print "_stroke_move", self._mouse_motion ,self.study_mode_flag
            c = self.canvas
            self.minitouch.touch_end()
            cavas_x, cavas_y = (int(c.canvasx(event.x)), int(c.canvasy(event.y)))
                    ...
                    if self._mouse_motion_crop == "crop":
                        ...
                        try:
                            ...
                            if self.enable_script:
                                if tkMessageBox.askokcancel('提示', '对此区域操作：点击选<是>,检查选<否>'):
    ...
    def crop_image_show(self):
        ...
        # tkMessageBox.showinfo(title="提示框", message="用鼠标在右侧屏幕上画出要截取的位置，方框内图像即可保存到本地文件maintmp_crop.png")
    ...
    def control_edit(self, mode="clicktext"):
        ...
        if mode == "clicktext":
            ...
            self.textout.insert(INSERT, "settext:12345\n")
    ...
    def control_save(self, event):
        try:
            ...
            # self.textout.update()
            (path, shotname) = os.path.split(ref)
            if tkMessageBox.askokcancel('提示', '是否保存到 Json 用例文件？'):
    ...
    def enable_root(self):
        try:
            ...
            serial = [self.serial]
            if tkMessageBox.askokcancel('提示', 'root所有设备点<是>,<否>仅仅root当前设备:' + self.serial):
                serial = self.getAdb2()
            else:
                pass
            for i in xrange(0, len(serial)):
    ...
    def enable_wifi(self, tp, ps):
        try:
            self.inidevice()
            serial = [self.serial]
            # if tkMessageBox.askokcancel('提示', '连接所有设备wifi点<是>,<否>仅连接当前设备:' + self.serial):
            #     serial = self.getAdb2()
            # else:
            #     pass
            for i in xrange(0, len(serial)):
    ...
    def install_app(self):
        ...
        if len(serial) > 1:
            if tkMessageBox.askokcancel('提示', '安装应用到所有设备<是>,<否>仅安装到当前设备:' + self.serial):
                pass
            else:
                serial = [self.serial]
        for fl in (fn):
    ...
    def push_res(self):
        try:
            serial = [self.serial]
            if tkMessageBox.askokcancel('提示', 'push到所有设备sdcard点<是>,<否>仅仅push到当前设备:' + self.serial):
                serial = self.getAdb2()
            else:
                pass
    ...
    def emmc_start_tool(self, serial):
        ...
        st = datetime.datetime.now()
        if len(serial) > 1:
            if tkMessageBox.askokcancel('提示', '所有设备执行点<是>,<否>仅仅执行当前设备:' + self.serial):
                pass
            else:
                serial = [self.serial]
        for i in xrange(0, len(serial)):
    ...
    def launche_test(self):
        ln = tkMessageBox.askokcancel('提示', '测试后，是否需要卸载应用？')
    ...
    def update_myself(self):
        try:
            ...
            if self.versionCompare(VERSION, gfx_version) == 2:
                if tkMessageBox.askokcancel('提示', 'TTMS上发现新版本:' + str(gfx_version) + '，请更新！'):
            ...
            # import traceback
            # traceback.print_exc()
            tkMessageBox.askokcancel('提示', 'TTMS连接不上，请查看网络状况!')
        finally:
    ...
    def compareImage(self, i1, i2, li=False, bc=False):
        # t = threading.Thread(target=self.device_info_thread,args=(self.serial,)).start()
        otcimage = videotest.OTCImage(self.d)
        b = self.d(resourceId=i1).info["bounds"]  # 2个坐标位置图片对比
        box = (b["left"], b["top"], b["right"] - b["left"], b["bottom"] - b["top"])
        # box=(0,198,769,870-198)
        if li:  # 如果是本地对比，i1为本地图片，i2为手机资源位置图片
            img_0 = Image.open(i1)
        else:
    ...
    def parserNode(self, nc, node, x, y):
        for i in xrange(0, len(node)):
            ...
            if ("android.widget.TextView" == className) or ("android.widget.ImageView" == className) or (
                        "android.widget.EditText" == className) or ("android.widget.Switch" == className) or \
                    ("android.widget.ImageButton" == className) or ("android.widget.Button" == className):
    ...
    def d_dump_threading(self, x, y):
        try:
            self.textout.delete(1.0, END)
            t = self.d.dump("test.xml", compressed=False)
            tree = ET.parse("test.xml")
            t = self.xmlParser(tree, x, y)
        except Exception, e:
            import traceback
            traceback.print_exc()
            self.d.dump("test.xml", compressed=False)
            tree = ET.parse("test.xml")
            t = self.xmlParser(tree, x, y)
        finally:
            if type(t) == list:
                # hm_0=int(self.WIDTH) + int(self.HEIGHT)
                hm_0 = 1900
                for i in t:
                    box = i.get('bounds')
                    if len(box) > 1:
                        t1 = box.replace("][", ",").replace("]", "").replace("[", "").split(",")
                        bound_x = int(t1[0]) + (int(t1[2]) - int(t1[0])) / 2
                        bound_y = int(t1[1]) + (int(t1[3]) - int(t1[1])) / 2
                        hm = abs(x - bound_x) + abs(y - bound_y)
                        i['hm'] = hm
                        # print hm
                        if hm < hm_0:
                            hm_0 = hm
                ...
                for i in t:
                    if i['hm'] == hm_0:
                        print i['text'], i['class'], i['resource-id'], i['bounds']
                        self.textout.insert(INSERT, "text: " + i["text"] + "\nresourceId: " + i[
                            "resource-id"] + "\nclassName: " + i[
                                                "class"] + "\nbounds: " + i["bounds"] + "\n")
    ...
    def study_mode_threading(self):
        ...
        self.textout.delete("1.0", END)
        self.textout.insert(END, u"界面获取模式: " + (u"开启" if self.study_mode_flag else u"关闭") + "\n")
        if self.study_mode_flag:
            threading.Thread(target=self.study_mode).start()
            tkMessageBox.askokcancel('提示', '界面获取模式（取消需再次点击获取按钮）')

    def study_mode(self):
        # cmd = ('adb -s ' + self.serial + ' forward tcp:9008 tcp:9008')
        # subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        # subprocess.Popen(['adb', '-s', self.serial, 'shell', "am", "instrument", "-w",
        #                   "com.github.uiautomator.test/android.support.test.runner.AndroidJUnitRunner"],
        #                  stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        self.d.dump("test.xm", compressed=False)
    ...
    def watch_threading(self):
        t = threading.Thread(target=self.watch)
        t.start()
        t.join()

    def watch(self):
        self.d.watcher("AUTO_PER").when(text=u"允许").click(text=u"允许")
        self.d.watcher("AUTO_PERE").when(text="ALLOW").click(text="ALLOW")
        self.d.watchers.run()

    def watch_define_threading(self):
        try:
            self.logger = MyLogger("watch_log")
            watchtext = dl.askstring("监视", "text=xx or id=xx", initialvalue="")
            if "" == watchtext:
                watchtext = "id=android:id/topPanel"
            t = threading.Thread(target=self.watch_define, args=(watchtext,))
            t.start()
        except Exception, e:
            import traceback
            traceback.print_exc()

    def watch_define(self, res):
        while True:
            self.d.handlers.on(self.pan_hand(self.d, res))
            time.sleep(2)

    def pan_hand(self, device, res):  # "id=android:id/alertTitle","id=android:id/topPanel
        try:
            res1 = res
            if "text=" in res:
                if device(text=res.split("=")[1]).exists:
                    print "pop text window"
                    device.screenshot("pic/pop_alert_" + time.strftime("%m%d%H%M%S", time.localtime()) + ".png")
            else:
                if device(resourceId=res.split("=")[1]).exists:
                    print "pop resid window"
                    self.logger.info("发现弹窗")
                    if device(resourceId="android:id/alertTitle").exists:  # 如果有标题就必须有分割线
                        self.logger.info("发现标题窗")
                        if device(resourceId="android:id/titleDividerline").exists:
                            get = self.find_template(device, "icon/pic/wiko_pop_devide_h.720x1512.png", threshold=0.99,
                                                     rgb=False, wt=3)
                            if get != None:
                                self.logger.info("检查分割条:Passed")
                            try:
                                if self.d(resourceId="android:id/button2").exists:
                                    b = self.d(resourceId="android:id/button2").bounds
                                    box = (b["left"], b["top"], b["right"], b["bottom"])
                                    # imgsrc = self.minicap_ins.crop_image()
                                    imgsrc = Image.open(
                                        self.d.screenshot(str(self.d.server.adb.device_serial()) + "_tmp.png"))
                                    imgsrc.save("tmp.png")
                                    img = Image.open("tmp.png")
                                    img = img.crop(box)
                                    color = GetMainColor.get_dominant_color(img)
                                    getcolor = GetMainColor.min_color_diff(color)
                                    if len(getcolor) == 2:
                                        if getcolor[1] == "GREEN":
                                            self.logger.info("检查按钮2颜色:Passed")
                                        else:
                                            self.logger.info("检查按钮2颜色:Failed")
                                ...
                                if self.d(resourceId="android:id/button1").exists:
                                    ...
                                    if len(getcolor) == 2:
                                        if getcolor[1] == "GREEN":
                                            self.logger.info("检查按钮1颜色:Passed")
                                        else:
                                            self.logger.info("检查按钮1颜色:Failed")
                                    print "getcolor:", getcolor
                            except Exception, e:
                                pass
                        else:
                            self.logger.error("检查分割条:Failed,请查看截图")
                        device.screenshot("pic/pop_alert_" + time.strftime("%m%d%H%M%S", time.localtime()) + ".png")
                    self.raw_cmd_nowait('shell', 'input', 'keyevent 4')
            return True
            # self.root.after(1000, self.watch_define(res1))
        except Exception, e:
            import traceback
            traceback.print_exc()
