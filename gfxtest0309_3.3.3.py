...
class GFXTest():
    ...
    def gettk(self):
        ...
        # edit_menu.add_command(label="编辑", command=self.control_edit)
        # edit_menu.add_command(label="保存", command=self.control_edit)
    ...
    def help(self):
        try:
            ...
            self.textout.insert("1.0", "Help please refer to PPT. \n")
            self.textout.insert("1.0", "Download:\n http://ttms.tinno.com/tools/test-tools-version/24/\n")
            self.textout.insert("1.0", "Email：\n lin.shen@tinno.com\n")
            self.textout.update()
    ...
    def getLog(self, pkg):
        try:
            ...
            out = self.raw_cmd( 'shell', 'logcat', '-c')
    ...
    def installbundle(self):
        try:
            ...
            if "No such" in out1:
                ...
                out = self.raw_cmd( 'shell', 'ls /data/local/tmp/bundle.jar')
                time.sleep(1)
            ...
            out = subprocess.check_output(
                ...
                stderr=subprocess.STDOUT, shell=True)
            if "No such" in out:
                out = self.raw_cmd( 'push', os.getcwd() + '/lib/bundle/busybox', '/data/local/tmp/')
                time.sleep(1)
            ...
            if "com.github.uiautomator.test" not in outinstallest:
                out = self.raw_cmd( 'install', '-r', os.getcwd() + '/lib/bundle/app-test.apk')
            cpu = self.raw_cmd( 'shell', 'getprop', 'ro.product.cpu.abi')
            cpu = cpu.strip()
            sdk = self.raw_cmd( 'shell', 'getprop', 'ro.build.version.sdk')
            sdk = sdk.strip()
            out = self.raw_cmd( 'push', os.getcwd() + '/lib/' + sdk + '/' + cpu + '/minicap.so',
                               '/data/local/tmp/')
            out = self.raw_cmd( 'push', os.getcwd() + '/lib/' + sdk + '/' + cpu + '/minicap',
                               '/data/local/tmp/')
            out = self.raw_cmd( 'shell', 'chmod', '777', '/data/local/tmp/minicap')
            return True
        except Exception, e:
            return False
    ...
    def screenShot(self, path):
        try:
            ...
            self.raw_cmd('shell', 'LD_LIBRARY_PATH=/data/local/tmp', '/data/local/tmp/minicap','-P %s' % params,'-S -s > /sdcard/maintmp.png')
    ...
    def swipesystemui(self):
        ...
        self.raw_cmd( 'shell', 'input', 'swipe', str(self.WIDTH - 50), str(self.HEIGHT / 2),
                     "50",
                     str(self.HEIGHT / 2))
        self.raw_cmd( 'shell', 'input', 'keyevent 4').communicate()[
            0]
        ...
        try:
            ...
            if self.scroll_xy == "m" or self.scroll_xy == "p" or self.scroll_xy == "pa":
                if self.scroll_xy == "p":
                    ...
                    self.screenShot(os.getcwd() + "/pic/" + self.package + str(datetime.datetime.now().strftime("%m_%d_%H_%M_%S")) + ".png")
                    ...
                    for i in xrange(1, int(self.screensave) + 1):
                        ...
                        #     # self.textout.update()
                        self.screenShot(
                            os.getcwd() + "/pic/" + self.package + str(datetime.datetime.now().strftime("%m_%d_%H_%M_%S")) + ".png")
                elif self.scroll_xy == "m":
    ...
    def grantPermission(self, pkg):
        ...
        out = self.raw_cmd( 'shell', 'pm', 'grant', pkg,
        ...
        try:
            ...
            if self._mouse_motion == "click":
                ...
                if ((stend - ststart[0]).total_seconds() >0.6) and (len(ststart)>0):
                    self.raw_cmd( 'shell', ' input', 'swipe', str(click_x), str(click_y),str(click_x), str(click_y),'500')
                else:
                    self.raw_cmd( 'shell', ' input', 'tap', str(click_x), str(click_y))
                ...
                if len(self._mouse_motion_xy) > 3:
                    ...
                    if self._mouse_motion_crop == "crop":
                        ...
                        self.raw_cmd( 'shell', ' input', 'swipe', str(x_start), str(y_start),
                                     str(x_end),
                                     str(y_end))
                        self._mouse_motion_xy = []
                        time.sleep(1)
    ...
    def control_edit(self):

        print self.textout.get("1.0",END)
        ref = self.fileEdit.get()
        if ref == "":
            ref = "temp_ctrl.txt"
        with open(ref, 'a+') as file:
            file.write(outstr + '\n')
