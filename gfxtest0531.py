...
class GFXTest():
    ...
    def gettk(self):
        ...
        port = int('111' + str(random.randint(1,9)))
        self.minitouch = MiniTouch.Minitouch(port)
        self.minitouch.teardown()
        self.minitouch_thread(port)
        ...
        try:
            ...
            try:
                for i in xrange(0, len(serial)):
                    ...
                    subprocess.Popen(cmds, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    subprocess.Popen(['adb', '-s', serial[i], 'shell', 'input', 'keyevent', '3'],
                                     stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def find_template(self, d,img, threshold=0.5, rgb=False, wt=10):
        ...
        for i in xrange(0, wt):
            try:
                # 第一步：校验图像输入
                im_source = ac.imread(dv.screenshot(os.getcwd() + "/tmp.png"))
    ...
    def watch_threading(self, res= None,tx=None,img = None):
        t = threading.Thread(target=self.watch, args=(res,tx,img,))
        t.start()
        t.join()

    def watch(self,res= None,tx=None,img = None):
        import random
        t = str(random.randint(1,1000))
        if res!=None:
            self.d.watcher(str(random.randint(1,1000))).when(resourceId=res).click(resourceId=res)
        if tx != None:
            self.d.watcher(str(random.randint(1,1000))).when(resourceId=res).click(resourceId=res)
        if img != None:
            rt = ac.find_template(img)
            if rt != None:
                # cof = float(rt['confidence'][0]) / rt['confidence'][1]
                if rt['result'][0] > 1 and rt['result'][1] > 1:
                    if float(rt['confidence']) > 0.65:
