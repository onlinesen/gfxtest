...
class GFXTest():
    ...
    def gettk(self):
        ...
        for i in psutil.pids():
            try:
                ...
                if p.name().lower().split(".")[0][0:7] == "gfxtest":
                    minicapport = minicapport + 1
            except Exception, e:
                continue
        shouwport = "131" + str(minicapport / 2)
        ...
        for i in xrange(len(serial)):
            ...
            self.radiobutton.append(
                Radiobutton(self.root, bg="White", text=serial[i] + "_" + str(model), variable=self.radionButton_value,
                            value=serial[i],
                            command=lambda: self.on_serial_select(self.root, touchport)))
        ...
        device_tool.add_separator()
        device_tool.add_command(label="重连显示", command=lambda: self.on_minicap_reconnect(minicapport))
        ...
        self.minicap_ins = MyMini.MyMini(serial=self.serial, tk=self.root, cav=self.canvas, port=shouwport)
        self.minicap_ins.screen_simple()
        ...
        touchport = int('111' + str(int(minicapport / 2)))
        self.minitouch = MiniTouch.Minitouch(touchport)
        self.minitouch.teardown()
        self.minitouch_thread(touchport)
    ...
    def on_serial_select(self, tk, touchport):
        try:
            miniport = touchport
            self.serial = self.radionButton_value.get()
            size = self.screenSize(self.serial)
            ...
            time.sleep(1)
            self.minitouch.teardown()
            self.minitouch_thread(miniport)
            self.minitouch.install_and_setup(self.serial, self.WIDTH, self.HEIGHT)
        except Exception, e:
            self.minitch_flag = False
            import traceback
    ...
    def on_serial_refresh(self, tk, touchport):
        touchport = touchport
        self.textout.bind("<KeyPress-Return>", self.adb_mode)
        self.textout.bind("<KeyPress-Return>", self.adb_mode)
        self.textout.bind("<Control-KeyPress-s>", self.control_save)
        ...
        self.on_serial_select(self.root, touchport)
            ...
            for index, line in enumerate(playcontent):
                if line != "":
                    ...
                    if loopflag and "end" not in line:
                        ...
                        if "long" in line:
                            self.minitouch.touch((x, y), 2)
                            self.minitouch.touch_end()
                        else:
                            print x,y
                            self.minitouch.touch((x, y), 0.1)
                            self.minitouch.touch_end()
                        time.sleep(1)
                        ...
                        self.textout.insert(1.0,
                                            " ." + str(index) + ". " + serial + " Click Screen:" + str(x) +',' +str(y) +" Pass\n")
                        self.logger.info(str(index) + " " + serial + " Click screen:" + str(x) + "x" + str(y) + " Pass")
