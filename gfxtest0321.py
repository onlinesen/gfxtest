...
class GFXTest():
    ...
    def gettk(self):
        ...
        menubar.add_cascade(label='专项测试 ✈ |', menu=tools_menu)
        ...
        menubar.add_command(label="截图 〼 |", command=self.crop_image_show)
        screen_tool = Menu(menubar, tearoff=0)
        screen_tool.add_command(label="上滑", command=self.swiptDown)
        screen_tool.add_command(label="下滑", command=self.swiptUp)
        screen_tool.add_command(label="左滑", command=self.swiptLeft)
        screen_tool.add_command(label="右滑", command=self.swiptRight)
        ...
        menubar.add_cascade(label='手机常用命令 ㊣ |', menu=screen_tool)
        ...
        device_tool.add_command(label="断开设备", command=self.on_minicap_killed)
        device_tool.add_separator()
        device_tool.add_command(label="重新连接", command=self.on_minicap_reconnect)
        ...
        onekey_menu.add_command(label="Logcat", command=self.adb_log)
        onekey_menu.add_command(label="清除屏幕输出", command=self.clear_textout)
        onekey_menu.add_command(label="命令行工具", command=lambda: self.command_shell(serial))
        menubar.add_cascade(label='一键工具箱 ♨ |', menu=onekey_menu)
        ...
        menubar.add_cascade(label='脚本编辑', menu=edit_menu)
            ...
            if self.scroll_xy == "m" or self.scroll_xy == "p" or self.scroll_xy == "pa":
                if self.scroll_xy == "p":
                    ...
                    for i in xrange(1, int(self.screensave) + 1):
                        ...
                        # xy = self.playatxthread(getfile, self.d, self.dm)
                        self.textout.insert(END, "播放次数:" + str(i) + ", 文件:" + str(shotname) + ",结果：" + str(
                            self.result) + "\n")
                        self.textout.update()
                        #
                        # if "Found at" in xy:
    ...
    def record(self):
        ...
        try:
            ...
            self.raw_cmd('pull', '/sdcard/' + ref, os.getcwd())  # visual_bars
            self.textout.delete(1.0, END)
    ...
    def playatxthread(self, playfile, device, devicemin):
        t = threading.Thread(target=self.playatx, args=(playfile, device, devicemin,))
        t.setDaemon(False)
        t.start()
        t.join()
        ...
        try:
            ...
            if self._mouse_motion == "click":
                ...
                if len(self._mouse_motion_xy) > 3:
                    ...
                    print ('_mouse_move position: %s', (x_start, y_start, x_end, y_end))
        ...
        try:
            ...
            for i in xrange(0, len(serial)):
                ...
                if "encryptString" in r.content:
                    ...
                    self.raw_cmd('shell', 'setprop', 'persist.tinno.debug', str(ency))
                    self.raw_cmd('shell', 'setprop', 'persist.qiku.adb.input', '1')
                    self.raw_cmd('root')
                    self.raw_cmd('remount')
                self.textout.insert(END, "Root :" + self.serial + " OK! \n")
                self.textout.update()
        except Exception, e:
